...
const inputFile = join(process.cwd(), 'storage/healthmeasures-batch3.json');
const existingIdsFile = join(process.cwd(), 'storage/existing-healthmeasures-batch3-ids.json');
const outputFile = join(process.cwd(), 'scripts/crawlee/insert-healthmeasures-batch3.sql');

console.log('📖 Reading HealthMeasures batch 3 scales data...\n');

const content = await readFile(inputFile, 'utf-8');
const scales: HealthMeasuresScale[] = JSON.parse(content);

console.log(`Found ${scales.length} scales in HealthMeasures batch 3 data`);
...
const newScales = scales.filter(scale => {
  const scaleId = `scale_hm_${scale.id}`;
...
const sqlStatements: string[] = [
  '-- Auto-generated SQL insert statements for ecoa_scale from HealthMeasures Batch 3',
  '-- (GI, Dyspnea, Sexual Function, Psychosocial, Pediatric, Heart Failure)',
...
for (const scale of newScales) {
  ...
  NULL, '${domains}', NULL,
  '[]', 'https://www.healthmeasures.net/explore-measurement-systems', 1,
  0, 0, NULL
);`;
...
console.log('\n✨ Next steps:');
if (newScales.length === 0) {
  console.log('  ⚠️  No new scales to import! All batch 3 scales are already in the database.');
} else {
  console.log('  1. Review the SQL file');
  console.log('  2. Query existing scale IDs (optional):');
  console.log(`     pnpm exec wrangler d1 execute xcoa --local --command="SELECT id FROM ecoa_scale WHERE id LIKE 'scale_hm_%'" --json > storage/existing-healthmeasures-batch3-ids.json`);
  console.log('  3. Re-run this script to check for duplicates (optional)');
  console.log('  4. Import to local database:');
  console.log('     pnpm exec wrangler d1 execute xcoa --local --file=scripts/crawlee/insert-healthmeasures-batch3.sql');
  console.log('  5. Import to remote database:');
  console.log('     pnpm exec wrangler d1 execute xcoa --remote --file=scripts/crawlee/insert-healthmeasures-batch3.sql');
}
