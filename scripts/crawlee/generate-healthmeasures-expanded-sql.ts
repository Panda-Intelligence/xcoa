...
const inputFile = join(process.cwd(), 'storage/healthmeasures-expanded.json');
const existingIdsFile = join(process.cwd(), 'storage/existing-healthmeasures-ids.json');
const outputFile = join(process.cwd(), 'scripts/crawlee/insert-healthmeasures-expanded.sql');

console.log('📖 Reading HealthMeasures expanded scales data...\n');

const content = await readFile(inputFile, 'utf-8');
const scales: HealthMeasuresScale[] = JSON.parse(content);

console.log(`Found ${scales.length} scales in HealthMeasures expanded data`);
...
try {
  ...
  console.log(`Found ${existingIds.size} existing HealthMeasures scales in database`);
} catch (error) {
  console.log('⚠️  No existing HealthMeasures scales file found, will import all scales');
}
...
const sqlStatements: string[] = [
  '-- Auto-generated SQL insert statements for ecoa_scale from HealthMeasures (PROMIS Pediatric, Neuro-QoL, NIH Toolbox, ASCQ-Me)',
...
if (newScales.length === 0) {
  ...
  console.log('  2. Query existing HealthMeasures scale IDs:');
  ...
  console.log('     pnpm exec wrangler d1 execute xcoa --local --file=scripts/crawlee/insert-healthmeasures-expanded.sql');
  console.log('  5. Import to remote database:');
  console.log('     pnpm exec wrangler d1 execute xcoa --remote --file=scripts/crawlee/insert-healthmeasures-expanded.sql');
}
